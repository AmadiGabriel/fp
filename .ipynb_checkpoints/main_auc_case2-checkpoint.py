...
for rand in range(81,90):  
    ...
    indices0, indices1  = (np.linspace(0, len(Xy0), num=6, dtype=int), 
    ...
    tr_cv_0 = te_cv_0 =  pd.concat([cv_0_0, cv_1_0], axis = 0)
    tr_cv_1 = te_cv_1 =  pd.concat([cv_0_1, cv_1_1], axis = 0)
    tr_cv_2 = te_cv_2 =  pd.concat([cv_0_2, cv_1_2], axis = 0)
    tr_cv_3 = te_cv_3 =  pd.concat([cv_0_3, cv_1_3], axis = 0)
    tr_cv_4 = te_cv_4 =  pd.concat([cv_0_4, cv_1_4], axis = 0)
    ...
    kX0_traini = kX0_train.iloc[:,result_2_lgbm[maxrowindex]] #fi_thresh_iterest
    kX1_traini = kX1_train.iloc[:,result_2_lgbm[maxrowindex]] #fi_thresh_iterest
    kX2_traini = kX2_train.iloc[:,result_2_lgbm[maxrowindex]] #fi_thresh_iterest
    kX3_traini = kX3_train.iloc[:,result_2_lgbm[maxrowindex]] #fi_thresh_iterest
    kX4_traini = kX4_train.iloc[:,result_2_lgbm[maxrowindex]] #fi_thresh_iterest

    kX0_testi = kX0_test.iloc[:,result_2_lgbm[maxrowindex]] #fi_thresh_iterest
    kX1_testi = kX1_test.iloc[:,result_2_lgbm[maxrowindex]] #fi_thresh_iterest
    kX2_testi = kX2_test.iloc[:,result_2_lgbm[maxrowindex]] #fi_thresh_iterest
    kX3_testi = kX3_test.iloc[:,result_2_lgbm[maxrowindex]] #fi_thresh_iterest
    kX4_testi = kX4_test.iloc[:,result_2_lgbm[maxrowindex]] #fi_thresh_iterest
    ...
    Case_1_3_lgbm_fi_casc.to_csv(f'rand_auc_case2/new_lgbm_fi_casc_Case_2_misclass_rand_{str(rand)}.csv', index=False)
    ...
    df.to_csv(f'rand_ftpr_case2/new_lgbm_fi_casc_ftpr_Case_2_nr_misclass_rand_{str(rand)}.csv', index=False)
# %%
