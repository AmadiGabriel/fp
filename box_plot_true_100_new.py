...
for ijk in range(100):
    ...
    if permute == False:
        ...
        with open(f'text_files/permute_rand_{ijk}.txt', 'w') as f:
            f.write(str(max_auc))
...
directory = '.'  # Directory where your files are located
file_prefix = 'true_rand_'  # Prefix of the files you want to concatenate
file_extension = '.txt'  # Extension of the files you want to concatenate
concatenated_values = []

# List all files in the directory
files = os.listdir(directory)

# Filter files that start with the specified prefix and end with the specified extension
files = [f for f in files if f.startswith(file_prefix) and f.endswith(file_extension)]
...
for file in files:
    ...
    with open(file_path, 'r') as f:
        ...
        concatenated_values.append(content)
...
concatenated_values = [float(value) for value in concatenated_values]
...
boxprops = dict(facecolor='white', color='black', linewidth=1.5,  linestyle='--')
...
flierprops = dict(marker='+', markerfacecolor='red', markersize=8, linestyle='none',
                 markeredgewidth=1, markeredgecolor='black', linewidth=1.2)

plt.boxplot(concatenated_values, vert=True, widths=0.7, patch_artist=True, 
            boxprops=boxprops, medianprops=medianprops, whiskerprops=whiskerprops,
            flierprops=flierprops)

# Set additional parameters
# plt.ylim(0.935, 0.98)  # Set y-axis limits
# plt.axhline(y=lgbm_fi_iter_Problem_FPs["AUC_ROC"].max(),
#             color='black', linestyle='-.')  # Horizontal line at mean position
plt.scatter(1, 0.968, marker='*', color='k', s=100)
plt.xticks([])
plt.xlabel('True Labels')
plt.ylabel('AUC')
plt.grid(axis='both')
plt.tight_layout()
# plt.savefig(f'true_100.png', dpi=500, bbox_inches='tight')
plt.show()
